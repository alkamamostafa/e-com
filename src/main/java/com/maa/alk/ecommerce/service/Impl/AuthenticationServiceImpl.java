...
import com.maa.alk.ecommerce.domain.AuthProvider;
import com.maa.alk.ecommerce.domain.Role;
import com.maa.alk.ecommerce.domain.User;
import com.maa.alk.ecommerce.dto.CaptchaResponse;
import com.maa.alk.ecommerce.exception.ApiRequestException;
import com.maa.alk.ecommerce.exception.EmailException;
import com.maa.alk.ecommerce.exception.PasswordConfirmationException;
import com.maa.alk.ecommerce.exception.PasswordException;
import com.maa.alk.ecommerce.repository.UserRepository;
import com.maa.alk.ecommerce.security.JwtProvider;
import com.maa.alk.ecommerce.security.oauth2.OAuth2UserInfo;
import com.maa.alk.ecommerce.service.AuthenticationService;
    ...
    public Map<String, Object> login(String phone, String password) {
        try {
            ...
            User user = userRepository.findByPhoneNumber(phone)
                    .orElseThrow(() -> new ApiRequestException("phone not found.", HttpStatus.NOT_FOUND));
            String userRole = user.getRoles().iterator().next().name();
            String token = jwtProvider.createToken(phone, userRole);
            Map<String, Object> response = new HashMap<>();
            response.put("user", user);
            response.put("token", token);
            return response;
        } catch (AuthenticationException e) {
            throw new ApiRequestException(" Incorrect password or phone ", HttpStatus.FORBIDDEN);
        }
    }
